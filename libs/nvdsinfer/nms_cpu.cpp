...
#define error printf
                                 ...
                                 const int& width, const int& height)
    {
        ...
        output[0] = xAcc / scoreAcc + 0.5;
        output[1] = yAcc / scoreAcc + 0.5;
                ...
                const std::array<int, 4>& targetSize, const std::array<int, 4>& sourceSize)
    {
        try
        {
            // Sanity checks
            if (sourceSize.empty())
                printf("sourceSize cannot be empty. %d, %s, %s\n", __LINE__, __FUNCTION__, __FILE__);
            if (targetSize.empty())
                printf("targetSize cannot be empty. %d, %s, %s\n", __LINE__, __FUNCTION__, __FILE__);
            if (threshold < 0 || threshold > 1.0)
                printf("threshold value invalid. %d, %s, %s\n", __LINE__, __FUNCTION__, __FILE__);
